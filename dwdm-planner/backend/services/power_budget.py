def span_loss_db(length_km, atten_db_per_km, connectors, conn_loss_db, splices, splice_loss_db, amp_gain_db, amp_penalty_db):
    fiber_loss = length_km * atten_db_per_km
    connector_loss = connectors * conn_loss_db
    splice_loss = splices * splice_loss_db
    amp_effective_gain = max(0.0, amp_gain_db - amp_penalty_db)
    loss = fiber_loss + connector_loss + splice_loss - amp_effective_gain
    return round(loss, 2)


def total_loss_db(spans, atten_db_per_km, conn_loss_db, splice_loss_db):
    total = 0.0
    for span in spans:
        loss = span_loss_db(
